...
- 当前视觉测试图案为规整贴纸，直径约 `28mm`；棋片物理载体尺寸见 [`tech.md`](tech.md) / [`bom.md`](bom.md)。

## 当前架构

- 主仓库不再内置视觉标定、几何校正、识别或模型推理流程。
- 主仓库接收外部视觉系统产出的标准化 JSON 结果，也可在场景验证时调用 GhostVision 的 `capture-result` CLI 生成结果。
...
- `capture_pieces` 中每个 `slot` 必须在 `0..19`；`0..9` 对应外侧列 `y=10`，`10..19` 对应靠棋盘列 `y=9`。
